...
if (fs.existsSync(routesFile)) {
  ...
  const hasNotificationRoutes = routesContent.includes('NOTIFICATIONS_DIGESTS') && 
                               routesContent.includes('NOTIFICATIONS_PREFERENCES');
  console.log(`${hasNotificationRoutes ? '✅' : '❌'} Notification routes configured`);
...
const mjmlFile = path.join(__dirname, 'services/notification-svc/app/templates/weekly_wins.mjml');
if (fs.existsSync(mjmlFile)) {
  const mjmlContent = fs.readFileSync(mjmlFile, 'utf8');
  const hasRequiredSections = mjmlContent.includes('celebration_message') && 
                              mjmlContent.includes('hours_learned') &&
                              mjmlContent.includes('subjects_advanced') &&
                              mjmlContent.includes('goals_completed');
  console.log(`${hasRequiredSections ? '✅' : '❌'} MJML template has required sections`);
...
const analyticsTestFile = path.join(__dirname, 'services/analytics-svc/tests/test_weekly_wins.py');
const notificationTestFile = path.join(__dirname, 'services/notification-svc/tests/test_weekly_digest_send.py');

if (fs.existsSync(analyticsTestFile)) {
  const testContent = fs.readFileSync(analyticsTestFile, 'utf8');
  const hasTestMethods = testContent.includes('test_generate_weekly_wins') && 
                        testContent.includes('test_get_minutes_learned') &&
                        testContent.includes('test_celebration_highlight');
  console.log(`${hasTestMethods ? '✅' : '❌'} Analytics tests cover key methods`);
...
if (fs.existsSync(notificationTestFile)) {
  ...
  const hasNotificationTests = testContent.includes('test_send_digest_notification') && 
                              testContent.includes('test_process_subscriber_batch') &&
                              testContent.includes('test_send_email_digest');
  console.log(`${hasNotificationTests ? '✅' : '❌'} Notification tests cover key scenarios`);
...
const weeklyWinsFile = path.join(__dirname, 'services/analytics-svc/app/digests/weekly_wins.py');
if (fs.existsSync(weeklyWinsFile)) {
  const content = fs.readFileSync(weeklyWinsFile, 'utf8');
  const hasRequiredMetrics = content.includes('minutes_learned') && 
  ...
  console.log(`${hasRequiredMetrics ? '✅' : '❌'} Weekly wins includes all required metrics`);
  allFilesExist = allFilesExist && hasRequiredMetrics;
}

// Check scheduler functionality
const cronFile = path.join(__dirname, 'services/notification-svc/app/cron_weekly_digest.py');
if (fs.existsSync(cronFile)) {
  const content = fs.readFileSync(cronFile, 'utf8');
  const hasScheduling = content.includes('Sunday') && 
                       content.includes('17:00') &&
                       content.includes('timezone');
  console.log(`${hasScheduling ? '✅' : '❌'} Scheduler configured for Sunday 17:00 with timezone support`);
  allFilesExist = allFilesExist && hasScheduling;
}

// Check preferences UI
const preferencesFile = path.join(__dirname, 'apps/web/src/pages/notifications/Preferences.tsx');
if (fs.existsSync(preferencesFile)) {
  const content = fs.readFileSync(preferencesFile, 'utf8');
  const hasPreferences = content.includes('weeklyWinsEnabled') && 
                        content.includes('weeklyWinsDay') &&
                        content.includes('weeklyWinsTime') &&
                        content.includes('weeklyWinsTimezone');
  console.log(`${hasPreferences ? '✅' : '❌'} Preferences UI includes weekly wins settings`);
  allFilesExist = allFilesExist && hasPreferences;
}

console.log('\n' + '='.repeat(60));
if (allFilesExist) {
  console.log('🎉 ALL CHECKS PASSED! Weekly Wins Digest implementation is complete.');
  console.log('\n📋 Implementation Summary:');
  console.log('   ✅ Analytics digest generator with privacy-compliant data aggregation');
  ...
  console.log('   ✅ Metrics: minutes learned, subjects advanced, goals completed, SLP/SEL streaks');
