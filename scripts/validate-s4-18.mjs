...
function validateDockerfiles() {
  try {
    ...
    const result = execSync('(Get-ChildItem -Path services -Recurse -Name "Dockerfile").Count', {
      encoding: 'utf8',
      shell: 'powershell.exe'
    }).trim();
    
    const serviceCount = parseInt(result) || 0;
    if (serviceCount > 0) {
    ...
    // Fallback: check if services directory exists and has subdirectories
    try {
      if (fs.existsSync('services')) {
        const dirs = fs.readdirSync('services', { withFileTypes: true })
          .filter(dirent => dirent.isDirectory()).length;
        console.log(`✅ Services: Found ${dirs} service directories`);
        return dirs > 0;
      }
    } catch (e) {
      console.error('❌ Services: Error checking service directories');
      return false;
    }
  }
}
