...
class S310Validator {
  ...
  async checkGameClientAPI() {
    ...
    for (const method of requiredMethods) {
      const hasMethod = await this.fileContains(
        clientPath,
        new RegExp(`${method}\\s*[:(]`),
      );
      this.addResult(
    ...
    for (const interfaceName of requiredInterfaces) {
      const hasInterface = await this.fileContains(
        clientPath,
        new RegExp(`interface\\s+${interfaceName}`),
      );
      this.addResult(
        "Game Client Types",
  ...
  async checkGameComponents() {
    ...
    for (const component of components) {
      ...
      for (const feature of component.features) {
        const hasFeature = await this.fileContains(
          component.path,
      ...
      const hasPropsInterface = await this.fileContains(
        component.path,
        /interface.*Props/,
      );
      this.addResult(
  ...
  async checkRequiredFeatures() {
    ...
    for (const requirement of requirements) {
      const implemented = await this.fileContains(
        playPath,
        requirement.pattern,
      );
      this.addResult(
  ...
  printResults() {
    ...
    console.log(
      "3. Test with query params: /game/play?topic=math&difficulty=easy",
    );
