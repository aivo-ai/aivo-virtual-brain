...
export interface DunningState {
  status: 'payment_failed' | 'past_due' | 'subscription_canceled' | 'grace_period';
  lastFailedAt?: string;
...
export const getPlansWithDiscounts = (plans: Plan[], studentCount: number = 1): Plan[] => {
    ...
    queryFn: async (): Promise<DunningState> => {
      ...
      const scenarios = [
        ...
        },
        {
          ...
          message: 'Payment failed. We\'ll retry in 3 days. Update your payment method to avoid service interruption.',
          severity: 'warning' as const
        },
        {
          status: 'past_due' as const,
          lastFailedAt: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString(),
          ...
          message: 'Final payment attempt will be made on Aug 19. Please update your payment method immediately.',
...
export const useCreateCheckout = () => {
  ...
  return useMutation<CheckoutSession, Error, { planId: string; studentCount?: number }>({
    mutationFn: async ({ planId, studentCount = 1 }): Promise<CheckoutSession> => {
      // Use parameters to avoid linting errors
      console.log('Creating checkout for plan:', planId, 'students:', studentCount);
