...
export function useAdminAccess() {
  ...
  const hasStaffAccess = user?.roles?.includes('staff') || 
  ...
  const logDataAccess = async (learnerId: string, purpose: string, consentToken?: string) => {
  ...
  const logJobAction = async (jobId: string, action: 'requeue' | 'cancel' | 'retry', reason?: string) => {
